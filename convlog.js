...
sub url_decode {
    my ($str) = @_;
	JS::inline('let str') = $str;
	$str = JS::inline('decodeURIComponent(str)');
    return $str;
}
...
JS::inline('result') = $paipu;`

const compiled = p5pkg["Perlito5"].compile_p5_to_js([source])

function decodeTenhou(xml) {
	let result
	eval(compiled)

	return toJSObject(result)

	function toJSObject(perlObj) {
		if (perlObj != null && !Array.isArray(perlObj) && typeof perlObj === 'object') {
			if ('_array_' in perlObj) {
				perlObj = perlObj['_array_']
                let ret = []

				for (let i in perlObj) {
					ret.push(toJSObject(perlObj[i]))
				}

				return ret
			} else if ('_hash_' in perlObj) {
				perlObj = perlObj['_hash_']
                let ret = {}

				for (let key in perlObj) {
					ret[key] = toJSObject(perlObj[key])
				}
